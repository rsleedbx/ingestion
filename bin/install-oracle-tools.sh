...
if [ -w /opt/oracle ]; then
    ...
    if sudo chown -R $(logname):$(logname) /opt/oracle; then
        echo "sudo chown -R $(logname):$(logname); /opt/oracle."
    else
        echo "Error: sudo chown -R $(logname):$(logname) /opt/oracle exited with $?." 
        exit 1
    fi
fi
...
echo "checking for existance of /opt/oracle/bin/sqlplus" 
if [ -f "/opt/oracle/bin/sqlplus" ]; then
    echo "/opt/oracle/bin/sqlplus already setup. skipping"
else
    mkdir -p /opt/oracle
    cd /opt/oracle
    case "${machine_hardware}" in 
    x86_64)
        ...
        unzip -q instantclient-sqlplus-linux.x64-21.6.0.0.0dbru.zip
        mv instantclient_21_6/* bin/.
        rm -rf instantclient_21_6
        rm  instantclient-sqlplus-linux.x64-21.6.0.0.0dbru.zip
        ;;
      aarch64)
        wget https://download.oracle.com/otn_software/linux/instantclient/1919000/instantclient-sqlplus-linux.arm64-19.19.0.0.0dbru.zip
        unzip -q instantclient-sqlplus-linux.arm64-19.19.0.0.0dbru.zip
        mv instantclient_19_19 bin
        rm -rf instantclient_19_19 
        rm instantclient-sqlplus-linux.arm64-19.19.0.0.0dbru.zip
        ;;
    *)
        echo "Warning: ${machine_hardware} not handled for oracle sqlplus install"
        ;;
    esac      
fi
